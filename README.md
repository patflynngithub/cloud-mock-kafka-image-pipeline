...
- The images are not received from an external source. They are generated by the Kafka image receiving client, starting with the original image of a computer-drawn cute teddy bear that is stored in the *image_original/* subdirectory. After several of the exact same "generated" images in the image stream are received by the image receiving client, the most recently received image is changed by rotating it by 90 degrees and is used as the next image to be received from the image stream. This results in the Kafka image analysis client creating an image event of "image is different than the previous image."
